...
// function swapps keys with values in json data structure
var utils = (function () {
  function swap(json) {
    var ret = {};
    for (var key in json) {
...
class SetsConverter {
  ...
  ConvertString(text) {
    return text.toLowerCase().split("").map(this.convert);
  }
...
let unitConverter = new UnitConverter(morseCodeMap, utils.swap(morseCodeMap));
